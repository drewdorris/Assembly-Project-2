...
#define MAX_FULL_LENGTH 2000 // maximum amnt of characters in the file
...
int main(int argc, char *argv[]) {
	...
	char all[MAX_FULL_LENGTH]; // array will contain all text from file
	...
	while (fgets(buff, MAX_LINE_LENGTH - 1, file) != NULL) {
		...
		allIndex = allIndex - 1; // remove null terminator at end of each line
	}
	fclose(file);  // close the file after using it
	
	int count = -1;
	// count characters in all[]
	while (all[count + 1] != '\0') {
		count = count + 1;
	}
